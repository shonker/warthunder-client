	...
	while ( msg.message != WM_QUIT )
	{
...

		{
			...
			if ( !is_overlay_visible ||
				( ( overlay_rect.right - renderer::values::size_adjustment ) != ( game_rect.right - renderer::values::size_adjustment ) && ( overlay_rect.left - renderer::values::size_adjustment ) 
					!= ( game_rect.left - renderer::values::size_adjustment ) &&
					( overlay_rect.bottom - renderer::values::size_adjustment ) != 
					( game_rect.bottom - renderer::values::size_adjustment ) && 
					( overlay_rect.top - renderer::values::size_adjustment ) != 
					( game_rect.top - renderer::values::size_adjustment ) ) )
			{
				lf( SetWindowPos )( overlay_window, HWND_TOPMOST, ( game_rect.left - renderer::values::size_adjustment ), ( game_rect.top - renderer::values::size_adjustment ), 
					( game_rect.right - renderer::values::size_adjustment ) - ( game_rect.left - renderer::values::size_adjustment ), 
					( game_rect.bottom - renderer::values::size_adjustment ) - ( game_rect.top - renderer::values::size_adjustment ), SWP_SHOWWINDOW );
	...
	: game_window{ game_window }
{
	...
	overlay_window = lf( CreateWindowExW )( WS_EX_WINDOWEDGE | WS_EX_NOACTIVATE | WS_EX_NOREDIRECTIONBITMAP | WS_EX_LAYERED | WS_EX_TRANSPARENT, window_class.lpszClassName, 
		L"NZXT CAM", WS_BORDER | WS_POPUP | WS_CLIPSIBLINGS, 0u, 0u, 695, 1015, nullptr, nullptr, nullptr, nullptr );
